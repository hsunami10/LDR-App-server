...
module.exports = (app, pool) => {
  app.get('/api/partner/find-code/:code', isAuthenticated, wrapper(async (req, res, next) => {
  ...
  app.put('/api/partner/accept', isAuthenticated, wrapper(async (req, res, next) => {
    ...
    try {
      ...
      if (userID !== partnerID) {
        ...
        if (res2.rows.length === 0) {
          ...
          res.status(200).send({ success: false, message: 'You can only have one partner at a time.'});
        }
      } else {
        res.status(200).send({ success: false, message: 'You cannot be a partner with yourself!' });
      }
    } finally {
      client.release();
    }
