...
const notifications = (userID, filterQuery, lastID, lastData) => (
...
const users = (userID, filterQuery, orderCol, direction, lastID, lastData) => {
  ...
  return (
    `SELECT id, username, profile_pic, bio, date_joined, active, user_type, (SELECT COUNT(*) FROM friends WHERE user1_id = users.id OR user2_id = users.id) AS num_friends, (SELECT get_user_relation('${userID}', users.id)) AS type FROM users WHERE id != '${userID}' AND id != '' AND deleted = false AND ${lastID === '' ? '' : benchmark} (${filterQuery === '' ? 'true' : filterQuery}) ORDER BY ${orderQuery}, users.id ${direction} FETCH FIRST ${limit} ROWS ONLY`
  );
};
...
const topics = (userID, filterQuery, orderCol, direction, lastID, lastData) => {
  ...
  return (
    `SELECT id, name, lowercase_name, topic_pic, description, date_created, (SELECT COUNT(*) FROM topic_subscribers WHERE topics.id = topic_subscribers.topic_id) AS num_subscribers, (SELECT get_topic_relation('${userID}', topics.id)) AS type FROM topics WHERE ${lastID === '' ? '' : benchmark} (${filterQuery === '' ? 'true' : filterQuery}) AND id != '' ORDER BY ${orderQuery}, topics.id ${direction} FETCH FIRST ${limit} ROWS ONLY`
  );
};
